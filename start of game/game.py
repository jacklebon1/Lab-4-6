...
def remove_punct(text):
    ...
    input_no_punctuation = ""
    for char in text:
        if char not in string.punctuation:
            input_no_punctuation += char
    return input_no_punctuation
...
def normalise_input(user_input):
    ...
    text_normalise = remove_punct(user_input)
    text_normalise2=remove_spaces(text_normalise)
    lower_case=text_normalise2.lower()
    return lower_case
...
def is_valid_exit(exits, user_input):
    ...
    For example:print("Go " + direction.upper()+ " to " + leads_to + ".")
